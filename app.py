...
def webhook_handler():
    ...
    try:
        ...
        logger.debug(f"Request {request_id}: Processing combined text length = {len(combined_text)}")

        # Extract invoice data using LLM
        result = extract_invoice_data(combined_text)

        # Log the full extracted result
        logger.info(f"Request {request_id}: Extracted Invoice Data:\n{json.dumps(result, indent=2)}")

        # Check for error in response
        if "error" in result:
            logger.error(f"Request {request_id}: Extraction failed - {result['error']}")
            return jsonify({
                "status": "error",
        ...
        # Successful response
        logger.info(f"Request {request_id}: Invoice processed successfully")
